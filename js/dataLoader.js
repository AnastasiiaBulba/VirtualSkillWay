...
function loadHomeContent(data) {
  ...
  // Load ocean features
  const oceanFeatures = document.getElementById("ocean-features");
  if (oceanFeatures && data.ocean) {
    oceanFeatures.innerHTML = data.ocean.features
      .map(
        (feature) => `
            <div class="ocean-feature">
                <div class="ocean-feature-icon">${feature.icon}</div>
                <div class="ocean-feature-text">${feature.text}</div>
            </div>
        `
      )
      .join("");
      ...
      .map(
        ...
        <div class="news-card">
          <div class="news-image">
            ${
              update.image
                ? `<img src="${update.image}" alt="Ocean Level" class="news-image-photo" style="width:100%;height:auto;object-fit:cover;border-radius:12px;" />`
                : ""
            }
          </div>
      ...
      .map(
        ...
        <div class="news-card">
          <div class="news-image">
            ${
              diary.image
                ? `<img src="${diary.image}" alt="Ocean Level" class="news-image-photo" style="width:100%;height:auto;object-fit:cover;border-radius:12px;" />`
                : ""
            }
          </div>
          <div class="news-content">
            <span class="news-category diaries">Ocean Diary</span>
...
function loadFallbackContent() {
  ...
  const elements = {
    "hero-title": "Something below the Sea",
    "hero-description":
      "Dive into an underwater adventure where scientists battle ocean pollution with advanced drones, but discover something mysterious lurking in the depths.",
    "play-now-btn": "Play Now",
    "features-title": "Game Features",
    "how-to-play-title": "How to Play",
    "ocean-title": "Explore the Ocean Depths",
    "ocean-description":
      "Navigate through mysterious underwater environments and discover what lies beneath the surface.",
